...
class Agent:
    ...
    def __init__(self, color, agent_name, code_name, train: bool, reset: bool, backend: "AgentBackend"):
...
class AgentRunner:
    ...
    def __init__(self, train, agent_name, code_name, result_queue, reset):
...
class AgentBackend:
    ...
    def __init__(self, train, agent_name, code_name, result_queue, reset):
        self.train = train
        self.reset = reset
        #print('AgentBackend: self.train', self.train)
        self.code_name = code_name
        self.agent_name = agent_name
...
class SequentialAgentBackend(AgentBackend):
    ...
    def __init__(self, train, agent_name, code_name, reset):
        super().__init__(train, agent_name, code_name, queue.Queue(), reset)
        self.runner = None

    def start(self):
        self.runner = AgentRunner(self.train, self.agent_name, self.code_name, self.result_queue, self.reset)
...
def run_in_agent_runner(train: bool, agent_name: str, code_name: str, wta_queue: mp.Queue, atw_queue: mp.Queue, reset: bool):
    runner = AgentRunner(train, agent_name, code_name, atw_queue, reset)
    while True:
