...
def state_to_features(game_state: dict) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    bombs_location = [b[0] for b in bombs]
    enemies = game_state['others']
    
    print("game_state['bombs']", game_state['bombs'])
    print("self bomb", bomb)
    ...
    #print('Field no bombs: ', np.flip(np.flip(field, axis=0).T,axis=1))
    #print('Field with bombs: ', np.flip(np.flip(field_, axis=0).T,axis=1))
    ...
    if len(crates) > 0:      
        ...
        idx = np.argsort(np.sum(np.abs(crate_reldis), axis=1))[0:6] # look at the 6 closest crates
    ...
    if len(bombs)>0:
        ...
        for tile_path in free_tile_escape:
            ...
            for i in range(len(bombs_location)):
                ...
                #print('issafe:', issafe, "issafe == 0", issafe == 0, 'l path', len(tile_path), 'ticker', bombs_ticker[i])
                print('issafe:', issafe, "0 in issafe", 0 in issafe , 'l path', len(tile_path), 'ticker', bombs_ticker[i])
                if 0 in issafe  and len(tile_path)-1 <= bombs_ticker[i]: 
                    x.append(0)
                elif 0 in issafe and len(tile_path) > bombs_ticker[i]: 
                    # "Dead end"
                    x.append(1)
                else:
                    x.append(1)
...
def make_field_graph(field):
    ...
    :return: dict
    """
    print('Field inside make_field_graph: ', np.flip(np.flip(field, axis=0).T,axis=1))
    ...
    #print('zero vals coords inside make_field_graph: ', zero_vals)
    targets = []
