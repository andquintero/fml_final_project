...
def get_bomb_drop_weight(self):
    crate_num = 0
    if self.trainingXold[-1, 17] == 1 and self.trainingXnew[-1,17] == 0:
        crate_num = self.trainingXold[-1, 16]
    return crate_num
...
def reward_its_a_trap(self, events, new_game_state):
    ...
    # Check if Bomb action was done (17), and it was a bad spot (22)
    if self.trainingXold[-1, 22] == 0 and self.trainingXold[-1, 17] == 1 and self.trainingXnew[-1,17] == 0:
        events.append(ITS_A_TRAP)
    
    # Check if the seleced path was a dead end
    #print('Its a trap ', self.trainingXnew[-1, [0,1,2,3,23]])
    if all(self.trainingXnew[-1, [0,1,2,3,23]] == -1)  :
        events.append(ITS_A_TRAP)
...
def reward_moving_to_coin(self, events, new_game_state):
    ...
    if 'COIN_COLLECTED' not in events and 'COIN_FOUND' not in events:
        for i in range(tracking):
            #print('self.trainingXold[-1, coin_coords[i]: ', self.trainingXold[-1, coin_coords[i]])
            #print('self.trainingXnew[-1, coin_coords[i]]: ', self.trainingXnew[-1, coin_coords[i]])
...
def reward_moving_to_crate(self, events, new_game_state):
    ...
    if 'CRATE_DESTROYED' not in events and self.trainingXnew[-1, 18] == 0:
        for i in range(tracking):
